...
Skills are loaded in two phases to minimize token usage:

- **Essential skills** (always loaded): `scan_modes`, `coordination`, `root_agent`, `custom`
- **Deferred skills** (on-demand): `vulnerabilities`, `tooling`, `frameworks`, `technologies`

Deferred skills are loaded via the `load_skill` tool when the agent needs them, rather than being included in the initial system prompt.
